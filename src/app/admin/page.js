          ...
          <h1 className="text-2xl font-bold py-1">Recent Job Posts</h1>
          {
            ...
            careers.slice(1,limit ) .map(career=>{
              return <div key={career._id} className="w-4/5 p-5 border-2 bg-white rounded-md shadow-lg my-2 ">
                <h1 className="text-2xl font-semibold capitalize py-1">{career.career_title}</h1> 
                <h2 className="text-lg font-medium py-1">No. Of Positions: 
                  <span className="text-base capitalize"> {career.vacancyNumber}</span>
                </h2>
                <h2 className="text-lg font-medium py-1">Posted Date: 
                  <span className="text-base capitalize"> {career.posted_date}</span>
                </h2>
                <h2 className="text-lg font-medium py-1">Application Deadline: 
                  <span className="text-base capitalize"> {career.deadline}</span>
                </h2>
        ...
        <div className="w-full flex flex-col items-center">
          {
            ...
            projects.slice(1,limit ) .map(project =>{
              return <div key={project._id} className="p-5 bg-white border-2 rounded-md shadow-lg my-2 flex w-full items-center ">
                <h1 className="w-1/3">
                  <img src={`${API}/${project.project_image}`} alt={project.project_image} className="rounded-md w-full" style={{height:"300px"}}/>
                </h1>
                <div className="w-3/5 flex flex-col justify-center px-9"> 
                  <h1 className="text-2xl font-semibold capitalize py-1">{project.project_title}</h1> 
                  <h2 className="text-lg font-medium py-1">Category: 
                    <span className="text-base capitalize"> {project.category} </span> </h2>
                  <h2 className="text-lg font-medium py-1">Language: 
                    <span className="text-base font-normal capitalize"> {project.language} </span></h2>
                </div>
              </div>
            })
          }
